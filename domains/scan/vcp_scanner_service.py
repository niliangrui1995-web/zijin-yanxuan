...
class VcpScannerService:
    ...
    @staticmethod
    def _prepare_price_frame(df: pd.DataFrame | pl.DataFrame) -> pl.DataFrame:
    ...
    @staticmethod
    def _locate_current_trade_index(pldf: pl.DataFrame, current_day) -> int | None:
        ...
        if idx_query.height == 0:
            return None
        return idx_query.item()

    @staticmethod
    def _ensure_vcp_indicators(pldf: pl.DataFrame) -> pl.DataFrame:
        if "entangle" not in pldf.columns:
            return IndicatorService.calculate_indicators(pldf, include_chart=False)
        return pldf

    @staticmethod
    def _check_basic_entry_gates(
        pldf: pl.DataFrame,
        curr_idx: int,
        row: dict,
        rps120: float,
        ...
        if curr_idx < params.min_history_days - 1:
            return f"数据不足{params.min_history_days}天"
        ...
        if rps250 < params.rps_threshold:
            return f"长线动量未达标(RPS250:{rps250:.0f} < {params.rps_threshold})"
        if rps250 < 90 and rps250 < rps120:
            return f"短线背离长线(RPS250:{rps250:.0f} < RPS120:{rps120:.0f})"

        ma_slope_ok, ma_slope_val = VcpScannerService.check_ma_slope(pldf, curr_idx, params)
        if not ma_slope_ok:
            return f"50日均线斜率不足(当前{ma_slope_val:.4f} < {MIN_SMA50_SLOPE})"

        return None

    @staticmethod
    def _build_vcp_zone_context(pldf: pl.DataFrame, curr_idx: int, final_peaks: list) -> dict:
        peak_idx = final_peaks[0][0]
        last_peak_idx = final_peaks[-1][0]
    ...
    @staticmethod
    def _check_vcp_structure_rules(
        pldf: pl.DataFrame,
        curr_idx: int,
        row: dict,
        final_peaks: list,
        ...
        peak_idx = zone["peak_idx"]
        last_peak_idx = zone["last_peak_idx"]
        buy_zone = zone["buy_zone"]
        box_low = zone["box_low"]
        box_high = zone["box_high"]
        left_amp = zone["left_amp"]
        s1 = zone["s1"]
        s2 = zone["s2"]

        if curr_idx < last_peak_idx + MIN_DAYS_AFTER_LAST_PEAK:
            return f"买入点须在最后一峰之后{MIN_DAYS_AFTER_LAST_PEAK}个交易日"
        ...
        if buy_low and buy_low > 0:
            ...
            if buy_amp >= left_amp:
                return "买入区振幅未小于左侧区振幅"

        if left_amp > params.amp_threshold:
            return f"左侧区振幅超限({left_amp:.1%} > {params.amp_threshold:.0%})"
        ...
        if prior_250_start < peak_idx:
            ...
            if prior_250_max and prior_250_max > 0 and peak_high < prior_250_max * 0.92:
                ...
                return f"第一高点非前250日相对高点(偏离{deviation:.1f}%)"

        if len(final_peaks) >= 3:
            first_to_third_days = final_peaks[2][0] - final_peaks[0][0] + 1
            if first_to_third_days < MIN_FIRST_TO_THIRD_DAYS:
                return f"第一峰到第三峰不足{MIN_FIRST_TO_THIRD_DAYS}日(当前{first_to_third_days}日)"

        r1_len = zone["h2_idx"] - peak_idx + 1
        r2_len = zone["h3_idx"] - zone["h2_idx"] + 1
        if len(final_peaks) >= 3 and (r1_len + r2_len) <= MIN_R1_R2_DAYS:
            return f"R1+R2须大于{MIN_R1_R2_DAYS}个交易日(当前{r1_len + r2_len}日)"
        ...
        if len(final_peaks) >= 3:
            ...
            if r1_low > 0 and r2_low < r1_low:
                ...
                if drop_pct > MAX_R2_BELOW_R1_PCT:
                    return f"R2最低点相对R1最低点跌幅超15%(当前{drop_pct:.1%})"
            if r1_low > r2_low and box_low > 0:
                mid_left = (box_high + box_low) / 2
                if r1_low > mid_left:
                    return f"R1最低价高于R2时须在左侧区间下50%内(当前R1低{r1_low:.2f} > 左区中点{mid_left:.2f})"
    ...
    def _score_vcp_setup(
        ...
        return score, dist, breakout_status

    @staticmethod
    def _format_peak_dates(pldf: pl.DataFrame, final_peaks: list) -> list[str]:
        peak_dates = []
    ...
    @staticmethod
    def _resolve_buy_stage(curr_idx: int, last_peak_idx: int) -> str:
    ...
    def _build_vcp_metrics(
        ...
        metrics["买入阶段"] = VcpScannerService._resolve_buy_stage(curr_idx, last_peak_idx)
        return metrics
    ...
    def evaluate_conditions(
        ...
        pldf = VcpScannerService._prepare_price_frame(df)
        curr_idx = VcpScannerService._locate_current_trade_index(pldf, current_day)
        if curr_idx is None:
            return False, "非交易日", {}
        ...
        pldf = VcpScannerService._ensure_vcp_indicators(pldf)
        row = pldf.row(curr_idx, named=True)

        failed_reason = VcpScannerService._check_basic_entry_gates(
            pldf,
            curr_idx,
            row,
            rps120,
        ...
        zone = VcpScannerService._build_vcp_zone_context(pldf, curr_idx, final_peaks)
        failed_reason = VcpScannerService._check_vcp_structure_rules(pldf, curr_idx, row, final_peaks, params, zone)
        if failed_reason is not None:
            return False, failed_reason, {}

        score, dist, breakout_status = VcpScannerService._score_vcp_setup(
            pldf,
            curr_idx,
            row,
            rps250,
        ...
        peak_dates = VcpScannerService._format_peak_dates(pldf, final_peaks)
