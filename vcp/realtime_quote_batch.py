...
def should_log_pressure(*, total_codes: int, pending_codes: int, cache_hits: int, now: float, last_logged_at: float) -> bool:
    return (
        (total_codes >= 60 or pending_codes >= 40 or cache_hits >= 20)
        and (now - float(last_logged_at or 0.0) >= 30.0)
    )
