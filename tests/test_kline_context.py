...
def test_embedded_scan_signal_matches_trimmed_code_without_rewriting_payload_code():
    payload = payload_module._extract_scan_signal_payload(
        {"_signals": [{"code": "002975", "source_tab": "scan"}]},
        " 002975 ",
    )

    assert payload["代码"] == " 002975 "
...
def test_build_kline_echarts_payload_dims_compressed_moving_averages(monkeypatch):
    ...
    assert "tradeMarkers" not in payload
...
def test_build_kline_html_hides_echarts_tooltip_panel():
    ...
    assert "tradeMarkers" not in html
...
def test_yaohei_kline_html_syncs_canvas_scrollbar_and_tabular_nums(monkeypatch):
    ...
    assert colors["bg_canvas"] == THEME_YAOHEI["BG_CANVAS"] == "#000000"
