...
def parse_recommendations(content: str) -> dict[str, dict]:
    result = {}
...
def load_report_payload(fpath: str) -> tuple[list[dict], dict[str, dict]]:
    ...
    if not parsed_from_json:
        ...
        rec_map = parse_recommendations(content)
...
def build_na_daily_rows(report_files: list[str]) -> tuple[list[dict], list[str], tuple[str, ...]]:
    ...
    for fpath in report_files:
        report_date, report_ts, _ = parse_report_identity(fpath)
        stocks, rec_map = load_report_payload(fpath)
...
class NADailyRefreshService(QObject):
    ...
    def _load_report_payload(self, fpath: str):
        return load_report_payload(fpath)
    ...
    def _parse_battle_report(self, content: str) -> list:
        return parse_battle_report(content)
