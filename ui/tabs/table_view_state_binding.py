...
def bind_table_view_state(
    owner, table, settings, header_state_savers: list, settings_key: str = "header_state", *, restore_sort: bool = True
) -> bool:
    """绑定表格列宽、列顺序和排序状态的自动恢复与防抖保存。"""
    try:
        store = TableViewStateStore(settings, settings_key=settings_key)
        return store.bind(owner, table, header_state_savers, restore_sort=restore_sort)
