...
class RtScanWorker(QThread):
    ...
    def _run_one_round(self, np):
        ...
        for code, quote in quotes.items():
            ...
            if last_close <= 0:
                ...
                if hist_df is not None and len(hist_df) > 0:
                    last_close = (
                        float(hist_df.iloc[-2]["close"]) if len(hist_df) > 1 else float(hist_df.iloc[-1]["close"])
                    )
            ...
            if is_special:
                ...
                if has_rps and rt_df is not None and len(rt_df) >= 60:
                    ...
                    ok, reason, m = self.engine.evaluate_conditions(
                        rt_df, eval_day, float(r120), float(r250), None, special_params
                    )
                    if ok:
        ...
        codes_need_cap = [
            sig["代码"] for sig in _all_sigs.values() if not sig.get("市值") and sig["代码"] not in self._cap_cache
        ]
        if codes_need_cap:
            try:
