        ...
        def _load_bg():
            ...
            try:
                self.mw.cache_manager.try_load_rps_from_disk(
                    self.mw.engine, 
                    set_status_callback=lambda msg: self.mw._call_in_ui(lambda: self.mw.lbl_status.setText(msg))
                )
            except Exception as e:
        ...
        def _check_asian_data_bg():
            ...
            if needs_update and os.path.exists(script_path):
                ...
                try:
                    ...
                    self.mw._call_in_ui(lambda: event_bus.sig_asian_klines_ready.emit())
                except Exception as e:
            ...
            try:
                if self.mw.data_provider.test_network(timeout=3):
                    ...
                    try:
                        ...
                        self.mw._call_in_ui(_refresh_watchlist_names)
                    except Exception as e:
                        log.error(f"[智能启动] 后台同步大盘代码名称映射表时失败: {e}")

                    self.mw._call_in_ui(lambda: self.mw._update_network_ui(True))
                    if hasattr(self.mw, '_on_smart_startup_online_done'):
                        self.mw._call_in_ui(self.mw._on_smart_startup_online_done)
                    self.mw._call_in_ui(self.auto_start_rt_if_ready)
                else:
